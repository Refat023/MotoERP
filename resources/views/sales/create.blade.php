...
@section('title', 'New Sale - Super Shop POS')
...
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        ...
        <h1 class="mb-0 d-inline"><i class="bi bi-plus-circle"></i> Fast Billingss</h1>
    </div>
    @if($heldSales->count() > 0)
        <div class="badge bg-warning text-dark p-2">
            <i class="bi bi-clock-history"></i> {{ $heldSales->count() }} Held Sale(s)
        </div>
    @endif
</div>
    ...
    <div class="col-lg-8">
        ...
        <div class="card mb-3 border-warning">
            <div class="card-header bg-warning border-bottom">
                <h5 class="card-title mb-0"><i class="bi bi-clock-history"></i> Resume Held Sales</h5>
            </div>
            <div class="card-body">
                <div class="list-group">
                    ...
                    <div class="list-group-item d-flex justify-content-between align-items-center">
                        <div>
                            <strong>Sale #{{ str_pad($held->id, 6, '0', STR_PAD_LEFT) }}</strong>
                            <small class="d-block text-muted">
                                {{ $held->customer?->name ?? 'Walk-in' }} • {{ $held->items->sum('quantity') }} items
                            </small>
                        </div>
                        <a href="{{ route('sales.resume', $held->id) }}" class="btn btn-sm btn-warning">
    ...
    <div class="col-lg-4">
        <form id="saleForm" action="{{ route('sales.store') }}" method="POST">
                ...
                <div class="card-body">
                    <div class="mb-2">
                        ...
                        <div class="input-group">
                            <input type="number" class="form-control" id="discount" name="discount" value="0" step="0.01" min="0">
                    ...
                    <small class="text-muted">Item discounts can be set when adding products</small>
                </div>
            </div>
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <div class="form-check form-switch mb-2">
                        <input class="form-check-input" type="checkbox" id="applyTax" name="apply_tax" value="1">
                        <label class="form-check-label" for="applyTax">
                            Apply {{ $taxRate }}% VAT
                        </label>
                    </div>
                    <div class="input-group">
                        <input type="number" class="form-control" id="taxRate" name="tax_rate" value="{{ $taxRate }}" step="0.01" min="0" max="100">
                        <span class="input-group-text">%</span>
                    </div>
                    <div class="mt-2">
                        <small class="text-muted">Tax Amount: <strong id="taxAmount">৳0.00</strong></small>
                    </div>
                </div>
            </div>
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <div class="btn-group w-100" role="group">
                        <input type="radio" class="btn-check" name="payment_method" id="paymentCash" value="cash" required checked>
                        <label class="btn btn-outline-primary" for="paymentCash">
                            <i class="bi bi-cash-coin"></i> Cash
                        </label>

                        <input type="radio" class="btn-check" name="payment_method" id="paymentCard" value="card">
                        <label class="btn btn-outline-primary" for="paymentCard">
                            <i class="bi bi-credit-card"></i> Card
                        </label>

                        <input type="radio" class="btn-check" name="payment_method" id="paymentMobile" value="mobile_banking">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <textarea class="form-control" name="notes" rows="2" placeholder="Add any special notes..."></textarea>
                </div>
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="bg-light p-2 rounded">
                    <small class="text-muted">Price: <strong id="modalPrice">৳0</strong></small>
                </div>
            </div>
...
barcodeSearchInput.addEventListener('input', function() {
    ...
    // Remove automatic search on input - only search on Enter or button click
});
...
async function performProductSearch() {
    ...
    if (query.length < 1) {
        resultsList.innerHTML = '<p class="text-muted">Enter at least 1 character to search.</p>';
    ...
    try {
        ...
        console.log('Making fetch request to:', `/sales/search-product?q=${encodeURIComponent(query)}`);
...
function updateCartTable() {
    ...
    if (cartItems.length === 0) {
        cartDiv.innerHTML = '<p class="text-muted text-center py-3">No items added</p>';
        return;
    }
