<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('exchanges', function (Blueprint $table) {
            $table->id();
            $table->foreignId('sales_return_id')->constrained('sales_returns')->onDelete('cascade');
            $table->foreignId('returned_product_id')->constrained('products')->onDelete('cascade');
            $table->integer('returned_quantity');
            $table->foreignId('new_product_id')->constrained('products')->onDelete('cascade');
            $table->integer('new_quantity');
            $table->decimal('price_difference', 12, 2)->default(0); // positive if customer pays extra, negative if we give discount
            $table->enum('price_difference_method', ['cash', 'credit_note', 'cancel'])->nullable();
            $table->text('notes')->nullable();
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('exchanges');
    }
};
